...
getCartID(): void {
  ...
  this.csvService.fetchCsvData('files/CartID.csv').subscribe({
    next: (csvData) => {
...
getOrderID(): void {
  ...
  // Fetch CSV data to check if the ID already exists
  this.csvService.fetchCsvData('files/OrderID.csv').subscribe({
    next: (csvData) => {
  ...
  setTimeout(() => {
    if (form.valid && this.CARTID) {
      ...
      const orderData = this.cartItems.map((item) => ({
        ...
        OrderNotes: this.userInfo.orderNotes
      }));
      ...
      const summaryData = {
        ...
        OrderStatus: 'Pending',
        OrderDate: this.currentDate,
        OrderUpdateDate: this.currentDate,
        OrderNotes: this.userInfo.orderNotes
      };
